...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="批量运行 baseline / case_refiner_only / both 三组微调与评估。"
    )
    ...
    parser.add_argument("--residual-uniform", type=float, default=0.2)
...
def resolve_run_root(run_root: Path | None) -> Path:
    ...
    return (PROJECT_ROOT / "outputs" / "case_weighting_refiner_ablation" / timestamp).resolve()
...
def build_experiments(
    ...
    residual_tag = f"ru{int(round(residual_uniform * 100)):03d}"
    seed_tag = f"seed{seed:03d}"
    return [
        ExperimentSpec(
            name=f"baseline_cw0_{residual_tag}_{seed_tag}",
        ...
        ExperimentSpec(
            name=f"case_refiner_only_{weight_tag}_cw1_{residual_tag}_{seed_tag}",
            case_weighting_enabled=True,
            apply_to="case_refiner_only",
            residual_uniform=residual_uniform,
            weight_min=weight_min,
            weight_max=weight_max,
        ),
        ExperimentSpec(
            name=f"both_{weight_tag}_cw1_{residual_tag}_{seed_tag}",
            case_weighting_enabled=True,
            apply_to="both",
            residual_uniform=residual_uniform,
            weight_min=weight_min,
...
def update_summary_csv(run_root: Path, runs: list[dict[str, Any]]) -> None:
    csv_path = run_root / "refiner_ablation_results.csv"
...
def main() -> None:
    ...
    experiments = build_experiments(seed, args.weight_min, args.weight_max, args.residual_uniform)

    run_root = resolve_run_root(args.run_root)
    run_root.mkdir(parents=True, exist_ok=True)
    generated_config_dir = run_root / "generated_configs"
    manifest_path = run_root / "refiner_ablation_manifest.json"
    ...
    manifest: dict[str, Any] = {
        ...
        "residual_uniform": float(args.residual_uniform),
    ...
    print(f"[INFO] residual_uniform={args.residual_uniform:.1f}")
    ...
    print("=" * 80)
    print("[DONE] baseline / case_refiner_only / both 三组对照实验已处理完成。")
    print(f"[DONE] 总目录: {run_root}")
    print(f"[DONE] 汇总 CSV: {run_root / 'refiner_ablation_results.csv'}")
