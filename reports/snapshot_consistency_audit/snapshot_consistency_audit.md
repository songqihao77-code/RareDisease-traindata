...
Generated at: `2026-04-29T23:51:20`
...
- Confirmed ignored config keys: resume.skip_finetune, resume.skip_pretrain.
- Multiple output snapshot mix risk: YES.
- Baseline recommendation: use this snapshot only by referencing its frozen manifest and output dir; do not use the mutable top-level config as the reproduction contract.

If a formal baseline is needed, first create a locked snapshot config/manifest and remove or explicitly validate ignored control-flow keys.
...
- frozen manifest: `D:\RareDisease-traindata\outputs\snapshot_consistency_audit\frozen_snapshot_manifest.json`
...
Full table: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\snapshot_inventory.csv`
...
## 7. Suspicious / Ignored Config Keys

Status summary: CONFIRMED_IGNORED=2, CONSUMED_OR_MENTIONED=29, SUSPICIOUS=5
...
| resume.skip_finetune | True | CONFIRMED_IGNORED | High | runner does not contain 'skip_finetune'; field can mislead rerun expectations |
| resume.skip_pretrain | True | CONFIRMED_IGNORED | High | runner does not contain 'skip_pretrain'; field can mislead rerun expectations |
...
## 8. Candidate-Checkpoint Consistency
...
| High | Confirmed ignored config key: resume.skip_finetune | runner source does not mention the key leaf | Remove the key or implement explicit warning/strict validation in the runner before using the config for formal runs. |
| High | Confirmed ignored config key: resume.skip_pretrain | runner source does not mention the key leaf | Remove the key or implement explicit warning/strict validation in the runner before using the config for formal runs. |
...
1. Create `configs/mainline_full_pipeline_hybrid_tag_v5.locked.yaml` or an equivalent locked manifest for the audited snapshot.
2. Add non-training config validation in the runner or CI to warn on ignored control-flow keys such as `resume.skip_pretrain` and `resume.skip_finetune`.
3. Require every final report to cite `run_manifest.json`, candidate metadata, final metrics source table, and frozen audit manifest.
4. Keep future audit outputs outside `outputs/mainline_full_pipeline*` to avoid overwriting experiment artifacts.

## 14. Commands Run

- `D:\python\python.exe -m compileall tools/audit_snapshot_consistency.py`
- `D:\python\python.exe tools/audit_snapshot_consistency.py --run-dir outputs/mainline_full_pipeline_hybrid_tag_v5 --top-config configs/mainline_full_pipeline.yaml --runner tools/run_full_mainline_pipeline.py --out-report-dir reports/snapshot_consistency_audit --out-output-dir outputs/snapshot_consistency_audit`

## 15. Generated Files

- Markdown report: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\snapshot_consistency_audit.md`
- frozen manifest: `D:\RareDisease-traindata\outputs\snapshot_consistency_audit\frozen_snapshot_manifest.json`
- repository_state: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\repository_state.csv`
- snapshot_inventory: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\snapshot_inventory.csv`
- manifest_consistency_checks: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\manifest_consistency_checks.csv`
- top_config_vs_snapshot: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\top_config_vs_snapshot.csv`
- suspicious_config_keys: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\suspicious_config_keys.csv`
- candidate_consistency: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\candidate_consistency.csv`
- final_result_traceability: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\final_result_traceability.csv`
- output_snapshot_comparison: `D:\RareDisease-traindata\reports\snapshot_consistency_audit\tables\output_snapshot_comparison.csv`
