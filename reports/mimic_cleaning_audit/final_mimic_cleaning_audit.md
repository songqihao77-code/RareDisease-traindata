...
- 本次报告实际读取的 cleaned 文件: `D:\RareDisease-traindata\LLLdataset\dataset\processed\mimic_test.csv`。

- 注意: 用户指定的 `D:\RareDisease-traindata\LLLdataset\dataset\processed\test\mimic_test.csv` 在最终重跑时不可读；本报告使用当前可读的 `D:\RareDisease-traindata\LLLdataset\dataset\processed\mimic_test.csv` 生成。若恢复指定文件，请按第 12 节命令重跑。
...
| HPO 大量丢失或 invalid/obsolete HPO | 22 | medium/high: 会降低 gold disease-HPO overlap 和检索证据质量 | B | 统一 HP:0000000 格式，恢复误删 HPO，必要时用 HPO OBO 处理 obsolete/alt_id |
...
| unmapped/obsolete label in hyperedge/index | 64 | critical/high: gold 不在疾病池或 KB 中时 exact retrieval 不可比 | A/B | 修复 MONDO obsolete/alt_id/replaced_by 映射，并记录版本 |
| train/test leakage 或 similar-case 重复 | 0 | 方向不固定: 泄漏通常虚高，split 不一致会让测试口径不可信 | A | 用 subject_id/hadm_id/note_id/text hash 做最终确认；不要覆盖已有 exact evaluation |
...
D:\python\python.exe tools\audit_mimic_cleaning.py --original D:\RareDisease-traindata\LLLdataset\dataset\mimic_test.csv --cleaned D:\RareDisease-traindata\LLLdataset\dataset\processed\mimic_test.csv --output-dir reports\mimic_cleaning_audit
```

生成文件:

- `final_mimic_cleaning_audit.md`
- `manifest.json`
